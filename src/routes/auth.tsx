...
function AuthPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (suRole === "student" && !suSchool.trim()) {
      toast.error("School is required for students.");
      return;
    }
    if (suRole === "teacher" && !suSchool.trim()) {
      toast.error("School is required for teachers.");
      return;
    }
            ...
            <TabsContent value="signup" className="space-y-4 mt-4">
              <form onSubmit={handleSignup} className="space-y-3">
                    ...
                    {[
                      {
                        value: "student",
                        label: "Student",
                        description: "Learn, fork, and build with peers.",
                      },
                      {
                        value: "teacher",
                        label: "Teacher",
                        description: "Publish lessons & guide students.",
                      },
                    ].map((option) => (
                      <label
                        key={option.value}
                  ...
                  <>
                    <Field label="School" icon={UserIcon}>
                      <Input
                        ...
                        placeholder="e.g. Oak Valley High"
                      />
                    </Field>
                    <Field label="Grade or year (optional)" icon={UserIcon}>
                      <Input
                        value={suGrade}
                        onChange={(e) => setSuGrade(e.target.value)}
                ...
                {suRole === "teacher" && (
                  <>
                    ...
                    <Field label="Subject taught (optional)" icon={UserIcon}>
                      <Input
                        value={suSubject}
                        onChange={(e) => setSuSubject(e.target.value)}
                ...
                <p className="text-xs text-muted-foreground">
                  Min 8 chars password. You can change details later in Settings.
                </p>
                <Button type="submit" className="w-full" disabled={busy}>
