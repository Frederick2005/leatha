...
function DiffPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const { data: c } = await supabase
        .from("lessons")
        .select(
          "id, title, content, parent_lesson_id, author:profiles!lessons_author_profile_fkey(username)",
        )
        .eq("id", lessonId)
        .maybeSingle();
      if (!c || cancelled) {
  ...
  if (!child)
    return <div className="p-10 text-center text-muted-foreground">Lesson not found.</div>;
  if (!parent) {
    return (
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <h1 className="text-2xl font-display font-semibold flex items-center gap-2">
        <GitBranch className="h-5 w-5 text-primary" /> Diff
      </h1>
      <div className="text-sm text-muted-foreground mt-1 flex flex-wrap gap-2">
        <Link
          to="/lessons/$lessonId"
          params={{ lessonId: parent.id }}
          className="px-2 py-0.5 rounded bg-muted font-mono text-xs hover:text-primary"
        >
          {parent.title} {parent.author && `· @${parent.author.username}`}
        </Link>
        <span>→</span>
        <span className="px-2 py-0.5 rounded bg-primary-muted text-primary font-mono text-xs">
          {child.title}
        </span>
      </div>
