...
function SchoolsPage() {
  ...
  const load = async () => {
    ...
    if (rows.length) {
      ...
      rows.forEach((r) => {
        r.school = map.get(r.school_id) ?? null;
      });
    }
    setMemberships(rows);
  };
  ...
  const join = async () => {
    ...
    if (!school) {
      setBusy(false);
      toast.error("No school found with that code");
      return;
    }
    const { error } = await supabase
      .from("teacher_schools")
      .insert({ teacher_id: user.id, school_id: school.id });
    setBusy(false);
    if (error) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        ...
        {memberships.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            You haven't joined any schools yet. You can keep teaching as a freelancer or request to
            join one above.
          </p>
        ) : (
            ...
            {memberships.map((m) => (
              <li key={m.id} className="flex items-center justify-between py-3">
                ...
                <div className="flex items-center gap-2">
                  {m.status === "pending" && (
                    <span className="text-xs inline-flex items-center gap-1 px-2 py-1 rounded-full bg-amber-500/10 text-amber-600">
                      <Clock className="h-3 w-3" /> Pending
                    </span>
                  )}
                  {m.status === "approved" && (
                    <span className="text-xs inline-flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-500/10 text-emerald-600">
                      <Check className="h-3 w-3" /> Approved
                    </span>
                  )}
                  {m.status === "rejected" && (
                    <span className="text-xs inline-flex items-center gap-1 px-2 py-1 rounded-full bg-destructive/10 text-destructive">
                      <X className="h-3 w-3" /> Rejected
                    </span>
                  )}
                  <Button size="sm" variant="ghost" onClick={() => leave(m.id)}>
                    Leave
                  </Button>
                </div>
              </li>
      ...
      <p className="text-xs text-muted-foreground">
        Need to administer a school? Ask an admin to create one for you in the{" "}
