...
function LessonPage() {
  ...
  const [forks, setForks] = useState<
    { id: string; title: string; author: { username: string } | null }[]
  >([]);
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 grid lg:grid-cols-[1fr_240px] gap-6">
      <article className="min-w-0">
        ...
        {lesson.parent && (
          <div className="rounded-md border border-primary-muted bg-primary-muted/30 px-3 py-2 text-xs mb-3 inline-flex items-center gap-2">
            ...
            <Link
              to="/lessons/$lessonId"
              params={{ lessonId: lesson.parent.id }}
              className="font-medium text-primary hover:underline"
            >
              {lesson.parent.title}
            </Link>
            {lesson.parent.author && (
              <span className="text-muted-foreground">by @{lesson.parent.author.username}</span>
            )}
            {" — "}
            <Link
              to="/lessons/$lessonId/diff"
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This cannot be undone. All forks remain but will lose their parent link.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
            ...
            <Dialog>
              ...
              <DialogContent>
                ...
                <Textarea
                  ...
                  placeholder="Spam, harmful content, plagiarism…"
                />
        ...
        {Array.isArray(lesson.attachments) && lesson.attachments.length > 0 && (
          <div className="mt-8 border-t border-border pt-6">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Download attached files and supporting materials for this lesson.
            </p>
      ...
      <aside className="space-y-4">
        <div className="rounded-lg border border-border bg-card p-4 sticky top-20">
          ...
          {forks.length > 0 && (
            <>
              <h3 className="text-xs uppercase font-mono tracking-wider text-muted-foreground mt-5 mb-2">
                Forks ({forks.length})
              </h3>
              <ul className="space-y-1.5 text-sm">
                {forks.slice(0, 6).map((f) => (
                  <li key={f.id}>
                    <Link
                      to="/lessons/$lessonId"
                      params={{ lessonId: f.id }}
                      className="hover:text-primary line-clamp-1"
                    >
                      {f.title}
                    </Link>
                    {f.author && (
                      <span className="text-xs text-muted-foreground">@{f.author.username}</span>
                    )}
                  </li>
                ))}
              </ul>
