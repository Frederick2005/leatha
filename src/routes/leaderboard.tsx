...
function LeaderboardPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      ...
      {loading ? <div className="mt-6 h-64 rounded-lg bg-muted animate-pulse" /> : (
          ...
          {rows.map((r, i) => (
            <Link key={r.id} to="/u/$username" params={{ username: r.username }} className="flex items-center gap-3 p-3 hover:bg-accent transition-colors">
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-muted-foreground">{r.lesson_count}L · {r.fork_received_count}F</p>
              </div>
            </Link>
          ))}
          {rows.length === 0 && <div className="p-10 text-center text-muted-foreground text-sm">No users yet.</div>}
        </div>
      )}
    </div>
  );
