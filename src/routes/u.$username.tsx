...
function ProfilePage() {
  ...
  if (loading)
    return (
      <div className="p-10">
        <div className="h-40 rounded-lg bg-muted animate-pulse max-w-3xl mx-auto" />
      </div>
    );
  if (!profile)
    return (
      <div className="p-10 text-center text-muted-foreground">User @{username} not found.</div>
    );
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-4 mt-3 text-xs text-muted-foreground">
            <span className="inline-flex items-center gap-1">
              ...
              <strong className="text-foreground">{profile.points}</strong> pts
            </span>
            <span>
              <strong className="text-foreground">{profile.lesson_count}</strong> lessons
            </span>
            <span className="inline-flex items-center gap-1">
              <GitFork className="h-3.5 w-3.5" />
              <strong className="text-foreground">{profile.fork_received_count}</strong> forks
              received
            </span>
            <span>
            ...
            <span className="inline-flex items-center gap-1">
              <Calendar className="h-3.5 w-3.5" /> joined {timeAgo(profile.created_at)}
            </span>
          </div>
        </div>
    ...
    <div className="grid sm:grid-cols-2 gap-2">
      {profiles.map((p) => (
        <Link
          ...
          className="flex items-center gap-3 p-3 rounded-lg border border-border bg-card hover:border-primary/40"
        >
